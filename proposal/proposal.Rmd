...
It will do so by developing an R package that uses the s2geometry
...
* uses the [s2geometry](https://s2geometry.io/) library to provide functions
for `st_intersects`, `st_contains`, `st_intersection`, `st_union` and all other
available binary predicates and geometry operations currently provided by `sf`, for simple
...
* `Rcpp` will be used to write wrapper functions around the `s2geomety` classes and methods, such that they work on simple feature geometries
